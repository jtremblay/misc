...
use RRNATagger::FastqDb;
use RRNATagger::FastaDb;
use File::Basename;
use Env qw/TMPDIR/;
use File::Temp;
...
foreach(@test_index){
	my $barcode_length;
	my $temp_barcode = new RRNATagger::FastaDb($_) or die("Unable to open Fastq file, $_\n");
...
foreach(@test_fastqs){
	...
	my $temp_fastq = new RRNATagger::FastqDb($_) or die("Unable to open Fastq file, $_\n");
...
foreach my $fastq (@fastqs){
	...
	my $fasta_in = new RRNATagger::FastaDb($index);
	...
	my $in = new RRNATagger::FastqDb($fastq) or die("Unable to open Fastq file, $fastq\n");
